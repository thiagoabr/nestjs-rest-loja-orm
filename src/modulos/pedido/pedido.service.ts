...
export class PedidoService {
    ...
    dadosDoPedido.itensPedido.forEach((itemPedido) => {
      ...
      if (itemPedido.quantidade > produtoRelacionado.quantidade) {
        throw new BadRequestException(
          `A quantidade solicitada (${itemPedido.quantidade}) é maior do que a disponível (${produtoRelacionado.quantidade}) para o produto ${produtoRelacionado.nome}.`,
        );
      }
    });
  }
  ...
  async listarPedidosByUsuario(usuarioId: string) {
    return this.pedidoRepository.find({
      where: {
        usuario: { id: usuarioId },
      },
